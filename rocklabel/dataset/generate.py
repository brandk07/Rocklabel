...
def _profile_phrase(name: str | None, cfg_hash: str) -> str:
    """How to name a config in an error message: by profile where there is one."""
    return (f"generation profile {name!r}" if name
            else f"an unnamed config (hash {cfg_hash[:12]}...)")


def check_manifest(out_dir: str, cfg: dict, profile: str | None = None) -> dict:
    """Return the existing manifest (or a fresh one). Refuse mixed configs.

    ``profile`` is recorded beside the config rather than inside it: the hash
    is the identity of the settings, and folding a name into it would give two
    spellings of the same settings two different dataset directories.
    """
    ...
    if manifest.get("config_hash") != new_hash:
        old = manifest.get("profile") or identify_profile(manifest.get("config_hash", ""))
        raise ManifestConflict(
            f"Dataset directory {out_dir!r} holds data built with "
            f"{_profile_phrase(old, manifest.get('config_hash', '?'))}, and this run "
            f"uses {_profile_phrase(profile, new_hash)}. One dataset directory is one "
            "way of cutting frames - pooling two would mix populations that are not "
            "comparable. Write to a different --out directory."
        )
    return manifest
...
def run_generate(mcap_path: str, labels_path: str, out_dir: str, cfg: dict,
    ...
    # Written every time rather than only on creation, so a dataset generated
    # before profiles existed gains its name the first time it is added to.
