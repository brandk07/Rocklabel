...
Three tools in `altslam/evaluate.py`, each answering a different question.

```python
from altslam.evaluate import surface_sharpness, revisit_error, error_vs_gap
```
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest altslam/tests -q     # 32 tests
```
...
- **Nothing under `rocklabel/` was modified.** altslam only reads from it. The
